...
for i in $current_directory*.torrent ; do transmission-remote 192.168.1.10:9010 -n 'transmission:hoanglong?88' -a $i; done
...
for i in  $current_directory*.txt ; do
     cat $i | grep -o "magnet:?xt[^']*" | while read -r line ; do
                transmission-remote 192.168.1.10:9010 -n 'transmission:hoanglong?88' --add "$line"
                # your code goes here
        done
done
