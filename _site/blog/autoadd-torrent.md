The script has been improve to automatically add torrent and convert epub (and auto sent to my kindle):
...
for i in $current_directory*.torrent ; do transmission-remote -n 'transmission_user:password' -a $i; done
...
for i in  $current_directory*.txt ; do
     cat $i | grep -o "magnet:?xt[^']*" | while read -r line ; do
                transmission-remote -n 'transmission_user:password' --add "$line"
                # your code goes here
        done
done
